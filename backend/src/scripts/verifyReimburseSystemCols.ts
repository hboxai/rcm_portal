  ...
  try {
    const cols = await pool.query(`
      ...
      WHERE table_schema='public' AND table_name='api_bil_claim_reimburse'
        AND column_name IN ('bil_claim_reimburse_id','submit_claim_id','upload_id','s3_pdf_url','ocr_status','ocr_log','created_at','updated_at')
    ...
    const sample = await pool.query(`
      SELECT r.bil_claim_reimburse_id, r.submit_claim_id, s.claim_id AS submit_id,
